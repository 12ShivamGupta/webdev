...
// get id of selected team
		...
		jQuery.getJSON(json_url, function(data){
			...
			jQuery.each(data, function(key, val){
				console.log(val[0]);
				                $("#stable").append("<thead><tr><td><b>SubCategory Id</b></td><td><b>SubCategory Name</b></td></tr></thead> <tbody>")
                                for(var i = 0 ; i < val.length ; i++)
                                {    
                                var obj = val[i];
                                console.log(obj.id);
                                console.log(obj.subCategoryName);
                                  $("#stable").append("<tr><td>"+obj.id+"</td><td>"+obj.subCategoryName+"</td></tr>")
                                }
				                $("#stable").append("</tbody>")
				});
		});
		...
		jQuery.getJSON(json_url, function(data){
			...
			jQuery.each(data, function(key, val){
				console.log(val[0]);
                                ...
                                console.log(obj.id);
                                console.log(obj.subCategoryName);
                                $("#scats").append("<option value='" + obj.id + "'>" + obj.subCategoryName + "</option>")
                                }
				});
		});
            });        
